...
public class CompositionConnector extends Connector {

    public CompositionConnector(DiagramElement source, DiagramElement target) {
        super(source, target);
    }
    ...
    public void draw(Graphics g) {
        ...
        // Custom drawing for the filled diamond head
        ...
        fillDiamondHead(g2d, x2, y2);  // Drawing filled diamond at the target end
        g2d.dispose();
    }

    private void fillDiamondHead(Graphics2D g2d, int x, int y) {
        int size = 10;
        int[] xPoints = {x, x + size, x, x - size};
        int[] yPoints = {y - size, y, y + size, y};
        g2d.setColor(Color.BLACK);
        g2d.fillPolygon(xPoints, yPoints, 4);  // Filled diamond head
    }
}
